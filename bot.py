...
{durability_line}"""
    ...
    async def handle_use_net(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Получаем рыбу для текущей локации и сезона (без фильтра по времени суток)
        available_fish = await _run_sync(db.get_fish_by_location, location, season, min_level=player.get('level', 0) or 0, apply_time_filter=False)
...
Удачной рыбалки! 🎣
    ...
    async def _execute_dynamite_blast(self, user_id: int, chat_id: int, guaranteed: bool = False, reply_to_message_id: Optional[int] = None) -> None:
        ...
        available_fish_all = await _run_sync(db.get_fish_by_location, location, season, min_level=player_level, apply_time_filter=False)
        ...
        for idx in range(1, DYNAMITE_BATCH_ROLLS + 1):
            ...
            while not found and rarity_idx >= 0:
                pool = [f for f in available_fish_all if f.get('rarity') == search_rarity and float(f.get('min_weight', 0)) <= max_fish_weight]
                if pool:
