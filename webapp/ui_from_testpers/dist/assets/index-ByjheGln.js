...
`,"<br>")}</p>
  ...
  `,document.body.appendChild(a),a}function ke(a,e=1600){setTimeout(()=>{a.style.transition="opacity 0.6s ease",a.style.opacity="0",setTimeout(()=>a.remove(),700)},e)}function xe(a,e){const t=document.getElementById("btn-achievements"),s=document.getElementById("btn-rating"),i=document.getElementById("btn-results");[t,s,i].forEach(r=>{r&&r.addEventListener("click",()=>{l.haptic("light"),r.classList.remove("bounce"),r.offsetWidth,r.classList.add("bounce"),r.addEventListener("animationend",()=>r.classList.remove("bounce"),{once:!0})})}),s&&a&&s.addEventListener("click",a),i&&e&&i.addEventListener("click",e)}function Le(a){const e=document.getElementById("select-trophy-btn");e&&(e.addEventListener("click",a),e.addEventListener("pointerdown",()=>{e.style.transform="scale(0.96)"}),e.addEventListener("pointerup",()=>{e.style.transform=""}),e.addEventListener("pointerleave",()=>{e.style.transform=""}))}const Z=[{id:"home",icon:"🧭",label:"ГЛАВНАЯ"},{id:"shop",icon:"🧺",label:"ЛАВКА"},{id:"friends",icon:"👤",label:"ДРУЗЬЯ"},{id:"guilds",icon:"🔱",label:"АРТЕЛИ"},{id:"book",icon:"📖",label:"КНИГА"}],E={name:"АКВАМАН_01",tag:"@aquaman01",level:15,xp:7200,maxXp:1e4,avatar:"🤿"},k={common:"#90e0ef",rare:"#00b4d8",legendary:"#f4a82e",aquarium:"#4cc9f0",mythical:"#ef476f",anomaly:"#7b2cbf"},Ce=()=>{var a,e;return((e=(a=window.Telegram)==null?void 0:a.WebApp)==null?void 0:e.initData)||""};async function h(a,e={}){const s={"Content-Type":"application/json","X-Telegram-Init-Data":Ce(),...e.headers||{}},i=await fetch(a,{...e,headers:s});if(!i.ok){const r=await i.json().catch(()=>({error:"Unknown error"}));throw new Error(r.error||`HTTP error! status: ${i.status}`)}return i.json()}const H=h;class Ee{constructor(){this.fillEl=null,this.wobbling=!1,this.profile={...E},this.el=this.build(),this.loadProfile()}async loadProfile(){try{const e=await h("/api/profile");e&&(this.profile.name=e.username||this.profile.name,this.profile.level=e.level||this.profile.level,this.profile.xp=e.xp||this.profile.xp,this.profile.maxXp=1e4,this.profile.tag=`@${e.user_id}`||this.profile.tag,this.updateUI())}catch(e){console.error("Failed to load profile:",e)}}updateUI(){const e=this.el.querySelector("#profile-name"),t=this.el.querySelector("#profile-tag"),s=this.el.querySelector(".level-label"),i=this.el.querySelector(".xp-label");e&&(e.textContent=this.profile.name.toUpperCase()),t&&(t.textContent=this.profile.tag),s&&(s.textContent=`Уровень ${this.profile.level}`),i&&(i.textContent=`${this.profile.xp.toLocaleString("ru")} XP`),this.animateProgress(0)}getElement(){return this.el}build(){const e=E,t=Math.round(e.xp/e.maxXp*100),s=document.createElement("div");s.id="profile-panel",s.className="profile-panel glass",s.innerHTML=`
    ...
    `,this.container.appendChild(e),setTimeout(()=>e.remove(),(i+.5)*1e3)}start(){this.intervalId=window.setInterval(()=>this.spawnOne(),380)}stop(){clearInterval(this.intervalId),this.container.querySelectorAll(".trophy-bubble").forEach(e=>e.remove())}restart(){this.stop(),this.start()}}const q=class q{static transitionTo(e,t){this.pendingTimeout&&(clearTimeout(this.pendingTimeout),this.pendingTimeout=0);const s=++this.transitionToken;e.style.transition="opacity 0.35s ease, transform 0.35s ease",e.style.opacity="0",e.style.transform="scale(0.93)",e.style.pointerEvents="none",t.style.display="flex",t.style.transition="none",t.style.opacity="0",t.style.transform="scale(1.06)",t.style.pointerEvents="all",this.pendingTimeout=window.setTimeout(()=>{s===this.transitionToken&&(e.style.display="none",e.style.opacity="",e.style.transform="",e.style.transition="",requestAnimationFrame(()=>{s===this.transitionToken&&(t.style.transition="opacity 0.38s cubic-bezier(0.25,0.46,0.45,0.94), transform 0.38s cubic-bezier(0.25,0.46,0.45,0.94)",t.style.opacity="1",t.style.transform="scale(1)")}))},300)}};q.transitionToken=0,q.pendingTimeout=0;let B=q;const Me={id:"no-trophy",emoji:"🐟",name:"Нет трофеев",latinName:"",rarity:"common",rarityLabel:"Обычная",rarityStars:"",weight:"0 кг",depth:"0 см"};class qe{constructor(e,t=2){this.cards=[],this.bubbleSpawner=null,this.fishData=[],this.startX=0,this.isDragging=!1,this.onChangeCallback=null,this.container=e,this.activeIndex=t,this.build(),this.bindSwipe(),this.update()}build(){this.container.innerHTML="",this.cards=[];const e=document.createElement("div");e.className="carousel__bubbles",this.container.appendChild(e),this.bubbleSpawner=new $e(e);const t=document.createElement("div");if(t.className="carousel__track",this.container.appendChild(t),this.fishData.length===0){t.innerHTML=`
      ...
      `;return}this.fishData.forEach((s,i)=>{const r=document.createElement("div");r.className="carousel__card",r.dataset.index=String(i);const n=k[s.rarity];r.innerHTML=`
        <div class="carousel__card-inner" style="--accent:${n}">
          ...
          <div class="carousel__rarity" style="color:${n}">${s.rarityStars} ${s.rarityLabel}</div>
    ...
    `,document.body.appendChild(t),t.addEventListener("click",s=>{s.target===t&&document.body.removeChild(t)}),t.querySelectorAll(".sell-opt-btn").forEach(s=>{s.addEventListener("click",()=>{const i=s.dataset.cat;l.haptic("light"),document.body.removeChild(t),i?this.sellBulk({category:i}):s.classList.contains("select-mode-btn")&&(this.mode="select",this.selectedIds.clear(),this.renderInventory())})})}async showFishDetailModal(e){var t,s,i;try{const r=await h("/api/inventory"),n=((r==null?void 0:r.items)||[]).filter(p=>e.ids.includes(p.id));if(n.length===0)return;const c=b(e.rarity),o=k[c]||"#ccc",d=document.createElement("div");d.className="modal-overlay is-open",d.style.display="flex";const v=`
      ...
      `;d.innerHTML=v,document.body.appendChild(d);let m=new Set,C=null;const X=d.querySelector("#fish-detail-content"),G=d.querySelector(".fish-action-buttons"),g=d.querySelector("#confirm-detail-btn");(t=d.querySelector("#make-trophy-btn"))==null||t.addEventListener("click",()=>{l.haptic("light"),C="trophy",G.style.display="none",X.style.display="block",g.style.display="block",g.textContent="СДЕЛАТЬ ТРОФЕЕМ"}),(s=d.querySelector("#select-to-sell-btn"))==null||s.addEventListener("click",()=>{l.haptic("light"),C="sell",G.style.display="none",X.style.display="block",g.style.display="block",g.textContent="ПРОДАТЬ"}),d.querySelectorAll(".fish-detail-card").forEach(p=>{p.addEventListener("click",()=>{l.haptic("light");const T=parseInt(p.dataset.id||"0");m.has(T)?(m.delete(T),p.classList.remove("selected")):(m.add(T),p.classList.add("selected"))})}),(i=d.querySelector("#cancel-detail-btn"))==null||i.addEventListener("click",()=>{l.haptic("light"),document.body.removeChild(d)}),g==null||g.addEventListener("click",async()=>{if(m.size===0){alert("Выберите хотя бы одну рыбу");return}l.haptic("medium"),document.body.removeChild(d),C==="trophy"?await this.makeTrophies(Array.from(m)):C==="sell"&&await this.sellBulk({ids:Array.from(m)})}),d.addEventListener("click",p=>{p.target===d&&document.body.removeChild(d)})}catch(r){console.error("Failed to load fish details:",r),alert("Ошибка загрузки деталей рыбы")}}async makeTrophies(e){try{let t=0;for(const s of e){const i=await h("/api/make-trophy",{method:"POST",body:JSON.stringify({id:s})});i!=null&&i.ok&&t++}t>0&&(l.haptic("success"),alert(`Создано трофеев: ${t}`),await this.loadInventory())}catch{alert("Ошибка при создании трофеев")}}async sellBulk(e){try{const t=await h("/api/sell-bulk",{method:"POST",body:JSON.stringify(e)});t&&t.earned_coins!==void 0&&(l.haptic("success"),alert(`Успешно продано!
...
Опыт: ${t.earned_xp} ✨`),this.mode="view",this.selectedIds.clear(),await this.loadInventory())}catch{alert("Ошибка при продаже")}}async loadInventory(){this.loading=!0;try{const e=await h("/api/inventory/grouped");this.items=(e==null?void 0:e.items)||[],this.renderInventory()}catch(e){console.error("Failed to load inventory:",e),this.renderError()}finally{this.loading=!1}}renderInventory(){const e=this.el.querySelector("#shop-content"),t=this.el.querySelector("#shop-sell-btn"),s=this.el.querySelector("#shop-select-actions");if(this.mode==="select"?(t.style.display="none",s.style.display="flex",this.updateSelectCount()):(t.style.display=this.items.length>0?"block":"none",s.style.display="none"),this.items.length===0){e.innerHTML=`
      ...
      <div class="inventory-grid">
        ${this.items.map((i,r)=>{const n=b(i.rarity),c=k[n]||"#ccc",o=i.ids.every(m=>this.selectedIds.has(m)),d=i.ids.some(m=>this.selectedIds.has(m))&&!o;let v="";return this.mode==="select"&&(o?v="is-selected":d&&(v="is-partial")),`
            <div class="inv-card glass ${v}" style="--card-color: ${c}" data-idx="${r}">
              ...
              <div class="inv-details">
                ...
                <div class="inv-rarity" style="color:${c}">${F(i.rarity)}</div>
                <div class="inv-weight">⚖️ ${i.total_weight.toFixed(2)} кг</div>
              </div>
              <div class="inv-price">
                <span>${i.price}</span> 🪙
              </div>
              ${this.mode==="select"?`<div class="select-indicator">${o?"✅":""}</div>`:""}
            </div>
          `}).join("")}
      </div>
    `,e.querySelectorAll(".inv-card").forEach(i=>{i.addEventListener("click",()=>{l.haptic("light");const r=parseInt(i.dataset.idx||"0"),n=this.items[r];this.mode==="select"?(n.ids.every(o=>this.selectedIds.has(o))?n.ids.forEach(o=>this.selectedIds.delete(o)):n.ids.forEach(o=>this.selectedIds.add(o)),this.renderInventory()):this.showFishDetailModal(n)})})}updateSelectCount(){const e=this.el.querySelector("#select-count");e&&(e.textContent=this.selectedIds.size.toString())}renderError(){const e=this.el.querySelector("#shop-content");e.innerHTML=`
      <div class="shop-empty">
    ...
    `,(e=this.el.querySelector("#guild-create-trigger"))==null||e.addEventListener("click",()=>{this.view="create",this.render(),l.haptic("medium")}),this.el.querySelectorAll(".guild-card").forEach(t=>{var s;(s=t.querySelector("button"))==null||s.addEventListener("click",async i=>{i.stopPropagation();const r=t.getAttribute("data-id");if(await We(r)){l.haptic("heavy");const c=y.find(o=>o.id===r);(c==null?void 0:c.type)==="invite"?alert("Заявка отправлена!"):await this.init()}else l.haptic("error"),alert("Не удалось вступить в артель.")})})}renderCreate(){var s,i;this.el.innerHTML=`
      ...
      <div class="glass artel-form" style="padding: 20px;">
        ...
        <div class="form-group">
          ...
          <div class="avatar-grid">
            ${J.map(r=>`
              <div class="avatar-opt ${this.selectedAvatar===r?"is-selected":""}" data-val="${r}">${u(r)}</div>
            `).join("")}
          </div>
        </div>
    ...
    `,(s=this.el.querySelector("#btn-leave"))==null||s.addEventListener("click",()=>{confirm("Вы уверены, что хотите покинуть артель?")&&(Ze(),l.haptic("medium"),this.render())}),this.el.querySelectorAll(".donate-btn").forEach(i=>{i.addEventListener("click",()=>{const r=parseInt(i.getAttribute("data-idx"),10),n=e.upgradeProgress[r];n.current=Math.min(n.required,n.current+Math.floor(Math.random()*5)+1),l.haptic("selection"),this.render()})}),this.el.querySelectorAll(".req-btn--yes").forEach(i=>{i.addEventListener("click",async()=>{const r=i.getAttribute("data-id");if(!r)return;const n=await Q(r,"accept");l.haptic(n?"heavy":"error"),n&&await this.init()})}),this.el.querySelectorAll(".req-btn--no").forEach(i=>{i.addEventListener("click",async()=>{const r=i.getAttribute("data-id");if(!r)return;const n=await Q(r,"decline");l.haptic(n?"medium":"error"),n&&await this.init()})})}}let P=[],x=[];async function $(){try{const a=await h("/api/friends");if(a&&a.ok){const e=Array.isArray(a.items)?a.items:[],t=Array.isArray(a.incoming_requests)?a.incoming_requests:[];P=e.map(s=>({id:String(s.user_id),name:s.username,level:Number(s.level||0),avatar:"👤",online:!!s.is_online,xp:Number(s.xp||0)})),x=t.map(s=>({id:String(s.request_id||s.id),name:String(s.username||"user"),level:Number(s.level||0),avatar:"👤"}))}}catch(a){console.error("Failed to load friends:",a)}}async function Ke(a){try{const e=await h("/api/friends/add",{method:"POST",body:JSON.stringify({username:a})});return!!(e!=null&&e.ok)}catch(e){return console.error("Failed to send friend request:",e),!1}}async function Qe(a){return pe(a,"accept")}async function et(a){return pe(a,"decline")}async function pe(a,e){try{const t=await h("/api/friends/request/respond",{method:"POST",body:JSON.stringify({request_id:a,action:e})});if(t!=null&&t.ok)return await $(),!0}catch(t){console.error("Failed to respond friend request:",t)}return!1}class tt{constructor(){this.currentView="list",this.loading=!1,this.el=this.buildShell()}getElement(){return this.el}buildShell(){const e=document.createElement("section");return e.id="screen-friends",e.className="screen friends-screen",e.setAttribute("role","main"),e}async init(){this.loading=!0,this.render(),await $(),this.loading=!1,this.render()}render(){if(this.loading){this.el.innerHTML='<div style="display:flex; justify-content:center; align-items:center; height:100%; color:white;">Загрузка...</div>';return}this.currentView==="requests"?this.renderRequests():this.renderList()}renderList(){var e,t;this.el.innerHTML=`
      ...
      <div class="friends-list" style="margin-top: 20px; display:flex; flex-direction:column; gap:10px;">
        ${P.length===0?'<p style="text-align:center; opacity:0.5; font-size:12px; margin-top:20px;">У вас пока нет друзей</p>':""}
        ${P.map(s=>`
    ...
    `,e}getElement(){return this.el}async init(){if(this.isOwner){const e=this.el.querySelector("#results-owner-panel");e&&(e.style.display="block")}this.bindEvents(),await this.loadResults()}bindEvents(){const e=this.el.querySelectorAll(".results-tab");if(e.forEach(t=>{t.addEventListener("click",()=>{const s=t.dataset.type;s===this.currentType||this.isLoading||(l.haptic("selection"),e.forEach(i=>i.classList.remove("is-active")),t.classList.add("is-active"),this.currentType=s,this.loadResults())})}),this.isOwner){const t=this.el.querySelector("#results-create-btn");t&&t.addEventListener("click",()=>this.createDraw())}}async loadResults(){if(this.isLoading)return;this.isLoading=!0;const e=this.el.querySelector("#results-list");e.innerHTML='<div class="results-loading">Загрузка...</div>';try{const t=await H(`/api/tickets/results?ticket_type=${this.currentType}`);if(!t.ok)throw new Error("Failed to load results");this.renderResults(e,t)}catch(t){console.error("Failed to load results:",t),e.innerHTML='<div class="results-error">Ошибка загрузки результатов</div>'}finally{this.isLoading=!1}}async createDraw(){var c;if(this.isLoading)return;const e=this.el.querySelector("#results-start-date"),t=this.el.querySelector("#results-end-date"),s=this.el.querySelector("#results-count"),i=e.value,r=t.value,n=parseInt(s.value)||3;if(!i||!r){l.showAlert("Укажите даты начала и окончания розыгрыша");return}if(new Date(i)>new Date(r)){l.showAlert("Дата начала не может быть позже даты окончания");return}this.isLoading=!0,l.haptic("medium");try{const o=await H("/api/tickets/draw",{method:"POST",body:JSON.stringify({ticket_type:this.currentType,start_date:i,end_date:r,count:n})});if(!o.ok)throw new Error("Failed to create draw");l.showAlert(`Розыгрыш проведен! Победителей: ${((c=o.items)==null?void 0:c.length)||0}`),await this.loadResults()}catch(o){console.error("Failed to create draw:",o),l.showAlert("Ошибка при проведении розыгрыша")}finally{this.isLoading=!1}}renderResults(e,t){if(!t.items||t.items.length===0){e.innerHTML=`
      ...
      <div class="results-period">
        ...
        <div class="results-period-dates">
          ${this.formatDate(s.start_date)} — ${this.formatDate(s.end_date)}
        </div>
        <div class="results-period-count">Победителей: ${s.count}</div>
      </div>
    `:"",r=t.items.map(n=>{const c=n.place===1?"🥇":n.place===2?"🥈":n.place===3?"🥉":"🎖️";return`
        <div class="results-winner ${n.place<=3?"results-winner-top":""}">
          <div class="results-winner-place">
            <span class="results-medal">${c}</span>
            <span class="results-place-number">#${n.place}</span>
          </div>
          <div class="results-winner-info">
            <div class="results-winner-username">${this.escapeHtml(n.username)}</div>
            <div class="results-winner-id">ID: ${n.user_id}</div>
            <div class="results-winner-ticket">Билет: <strong>${n.ticket_code}</strong></div>
          </div>
        </div>
      `}).join("");e.innerHTML=i+'<div class="results-winners">'+r+"</div>"}formatDate(e){try{return new Date(e).toLocaleDateString("ru-RU",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}}class rt{constructor(e){this.particles=[],this.W=0,this.H=0,this.rafId=0,this.COUNT=60,this.canvas=e,this.ctx=e.getContext("2d"),this.resize(),window.addEventListener("resize",this.resize.bind(this));for(let t=0;t<this.COUNT;t++){const s=this.createParticle();s.y=Math.random()*this.H,s.life=Math.floor(Math.random()*s.maxLife*.5),this.particles.push(s)}this.loop()}resize(){this.W=this.canvas.width=this.canvas.offsetWidth,this.H=this.canvas.height=this.canvas.offsetHeight}createParticle(){const e=Math.random()<.28;return{x:Math.random()*this.W,y:this.H+12,r:e?1.5+Math.random()*2:1.2+Math.random()*3,vx:(Math.random()-.5)*.4,vy:-(.2+Math.random()*.55),alpha:0,alphaTarget:.12+Math.random()*.4,life:0,maxLife:180+Math.random()*420,isSediment:e,wobble:Math.random()*Math.PI*2,wobbleSpeed:.02+Math.random()*.03}}drawBubble(e){const t=this.ctx;e.wobble+=e.wobbleSpeed;const s=e.x+Math.sin(e.wobble)*1.8;t.save(),t.globalAlpha=e.alpha,t.strokeStyle="rgba(160,220,255,0.65)",t.lineWidth=.7,t.beginPath(),t.arc(s,e.y,e.r,0,Math.PI*2),t.stroke(),t.fillStyle="rgba(255,255,255,0.35)",t.beginPath(),t.arc(s-e.r*.28,e.y-e.r*.28,e.r*.38,0,Math.PI*2),t.fill(),t.restore()}drawSediment(e){const t=this.ctx;t.save(),t.globalAlpha=e.alpha,t.fillStyle="rgba(180,150,90,0.7)",t.beginPath(),t.ellipse(e.x,e.y,e.r*1.6,e.r*.55,e.wobble,0,Math.PI*2),t.fill(),t.restore()}updateParticle(e){e.x+=e.vx,e.y+=e.vy,e.life+=1;const t=40,s=50;return e.life<t?e.alpha=e.life/t*e.alphaTarget:e.life>e.maxLife-s?e.alpha=(e.maxLife-e.life)/s*e.alphaTarget:e.alpha=e.alphaTarget,e.life<e.maxLife&&e.y>-10}loop(){this.ctx.clearRect(0,0,this.W,this.H);for(let e=0;e<this.particles.length;e++){const t=this.particles[e];this.updateParticle(t)?t.isSediment?this.drawSediment(t):this.drawBubble(t):this.particles[e]=this.createParticle()}this.rafId=requestAnimationFrame(this.loop.bind(this))}destroy(){cancelAnimationFrame(this.rafId),window.removeEventListener("resize",this.resize.bind(this))}}const nt=we();ye();const lt=document.getElementById("particles-canvas");new rt(lt);const ot=document.getElementById("bg-parallax");new Se(ot);const U=new Ee;U.updateFromTelegram();const ct=document.getElementById("profile-mount");ct.appendChild(U.getElement());async function dt(){try{await Promise.all([$(),L(),ue()]),console.log("Initial data loaded")}catch(a){console.error("Failed to load initial data:",a)}}dt();const ht=document.getElementById("carousel-mount"),M=new qe(ht,2),f=document.getElementById("screens-wrap"),Y=new Re;async function ut(){const a=await Pe();M.setFishData(a,O),Y.setItems(M.getItems())}ut();Y.onSelect(a=>{M.goTo(a)});Le(()=>{Y.open(M.getActiveIndex(),f)});const ve=new Ye,ee=ve.getElement(),te=document.getElementById("screen-book");te?te.replaceWith(ee):f.appendChild(ee);const me=new Xe,se=me.getElement(),ie=document.getElementById("screen-shop");ie?ie.replaceWith(se):f.appendChild(se);const fe=new Je,ae=fe.getElement(),re=document.getElementById("screen-guilds");re?re.replaceWith(ae):f.appendChild(ae);const D=new tt,ne=D.getElement(),le=document.getElementById("screen-friends");le?le.replaceWith(ne):f.appendChild(ne);const R=new st,pt=R.getElement();f.appendChild(pt);const z=new at,vt=z.getElement();f.appendChild(vt);let oe=!1,ce=!1,de=!1,I=!1,_=!1,A=!1;const mt=document.getElementById("tabbar-mount"),V=new ze(mt,f);V.onChange((a,e)=>{if(console.log(`Screen transition: ${a} -> ${e}`),e==="book"&&!oe&&(ve.init(),oe=!0),e==="shop"&&!ce&&(me.init(),ce=!0),e==="guilds"&&!de&&(fe.init(),de=!0),e==="friends"&&!I?(D.init(),I=!0):e==="friends"&&I&&D.init(),e==="rating"&&!_?(R.init(),_=!0):e==="rating"&&_&&R.init(),e==="results"&&!A?(z.init(),A=!0):e==="results"&&A&&z.init(),e==="home"){const t=document.getElementById("screen-home");t&&t.querySelectorAll(".slide-up").forEach(i=>{const r=i;r.style.animation="none",r.style.opacity="1",r.style.transform="translateY(0)"})}});xe(()=>V.switchTo("rating"),()=>V.switchTo("results"));setTimeout(()=>U.animateProgress(0),2e3);ke(nt,1600);
