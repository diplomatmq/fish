...
class Database:
    ...
    def init_db(self):
        ...
        with self._connect() as conn:
            ...
            # Таблица инвентаря драгоценностей
            cursor.execute('''
    ...
    def _run_migrations(self):
        ...
        with self._connect() as conn:
            ...
            ensure_column('players', 'consecutive_location_casts', 'INTEGER DEFAULT 0')
            ensure_column('players', 'last_cast_timestamp', 'TEXT')
    ...
    # ===== МЕТОДЫ РАБОТЫ С ДРАГОЦЕННОСТЯМИ =====
    
    def add_treasure(self, user_id: int, treasure_name: str, quantity: int = 1) -> bool:
        """Добавить драгоценность к игроку"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO player_treasures (user_id, treasure_name, quantity)
                    VALUES (?, ?, ?)
                    ON CONFLICT(user_id, treasure_name) DO UPDATE SET
                        quantity = quantity + ?
                ''', (user_id, treasure_name, quantity, quantity))
                conn.commit()
                return True
        except Exception as e:
            logger.exception(f'add_treasure failed for user {user_id}: {e}')
            return False
    
    def get_player_treasure_quantity(self, user_id: int, treasure_name: str) -> int:
        """Получить количество драгоценности у игрока"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT quantity FROM player_treasures
                    WHERE user_id = ? AND treasure_name = ?
                ''', (user_id, treasure_name))
                row = cursor.fetchone()
                return row[0] if row else 0
        except Exception as e:
            logger.exception(f'get_player_treasure_quantity failed: {e}')
            return 0
    
    def get_player_treasures(self, user_id: int) -> Dict[str, int]:
        """Получить все драгоценности игрока в виде словаря"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT treasure_name, quantity FROM player_treasures
                    WHERE user_id = ? AND quantity > 0
                ''', (user_id,))
                rows = cursor.fetchall()
                return {row[0]: row[1] for row in rows}
        except Exception as e:
            logger.exception(f'get_player_treasures failed: {e}')
            return {}
    
    def remove_treasure(self, user_id: int, treasure_name: str, quantity: int = 1) -> bool:
        """Удалить (по продаже/обмену) драгоценность у игрока"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                ...
                if not row or row[0] < quantity:
                    return False  # Недостаточно драгоценностей
                
                # Обновляем количество
                new_quantity = row[0] - quantity
                if new_quantity <= 0:
                    cursor.execute('''
                        DELETE FROM player_treasures
                        WHERE user_id = ? AND treasure_name = ?
                    ''', (user_id, treasure_name))
                else:
                    cursor.execute('''
                        UPDATE player_treasures SET quantity = ?
                        WHERE user_id = ? AND treasure_name = ?
                    ''', (new_quantity, user_id, treasure_name))
                
                conn.commit()
                return True
        except Exception as e:
            logger.exception(f'remove_treasure failed: {e}')
            return False
    
    def update_treasures(self, user_id: int, updates: Dict[str, int]) -> bool:
        """Массовое обновление драгоценностей (ключ=имя, значение=количество для добавления)"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                for treasure_name, quantity_delta in updates.items():
                    cursor.execute('''
                        INSERT INTO player_treasures (user_id, treasure_name, quantity)
                        VALUES (?, ?, ?)
                        ON CONFLICT(user_id, treasure_name) DO UPDATE SET
                            quantity = quantity + ?
                    ''', (user_id, treasure_name, quantity_delta, quantity_delta))
                conn.commit()
                return True
        except Exception as e:
            logger.exception(f'update_treasures failed: {e}')
            return False
    
    def add_diamonds(self, user_id: int, chat_id: int, amount: int) -> bool:
        """Добавить бриллианты игроку (обновляет поле diamonds в таблице players)"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
            ...
            logger.exception(f'add_diamonds failed: {e}')
            return False
    
    def subtract_diamonds(self, user_id: int, chat_id: int, amount: int) -> bool:
        """Вычесть бриллианты у игрока"""
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    UPDATE players SET diamonds = MAX(0, diamonds - ?)
                    WHERE user_id = ?
                ''', (amount, user_id))
                conn.commit()
                return True
        except Exception as e:
            logger.exception(f'subtract_diamonds failed: {e}')
            return False
    
    def get_diamonds(self, user_id: int, chat_id: int) -> int:
        """Получить количество бриллиантов у игрока"""
        try:
            player = self.get_player(user_id, chat_id)
            return player.get('diamonds', 0) if player else 0
        except Exception as e:
            logger.exception(f'get_diamonds failed: {e}')
            return 0

    # ===== МЕТОДЫ УПРАВЛЕНИЯ ПОПУЛЯЦИЕЙ РЫБ =====
    
    def update_population_state(self, user_id: int, location: str) -> tuple[bool, int, bool]:
        """
        Обновить состояние популяции при забросе.
        Возвращает (location_changed, consecutive_casts, show_warning).
        
        location_changed: Если true - игрок сменил локацию и счетчик сброшен
        consecutive_casts: Текущее количество забросов подряд на локации
        show_warning: Если true - показать предупреждение о малой популяции
        """
        try:
            from datetime import datetime
            ...
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT last_fishing_location, consecutive_location_casts, last_cast_timestamp
                ...
                last_loc, last_casts, last_time = row
                
                # Проверяем, прошло ли 60 минут - если да, сбрасываем штраф
                location_changed = False
                if last_time:
                    try:
                        last_dt = datetime.fromisoformat(last_time)
                        now_dt = datetime.fromisoformat(now)
                        mins_passed = (now_dt - last_dt).total_seconds() / 60
                        if mins_passed >= 60:
                            # Восстанавливаем популяцию
                            cursor.execute('''
                                UPDATE players 
                                SET last_fishing_location = ?, 
                                    consecutive_location_casts = 1,
                                    last_cast_timestamp = ?
                                WHERE user_id = ?
                ...
                # Проверяем, сменил ли локацию
                if last_loc != location:
                    location_changed = True
                    consecutive_casts = 1
                else:
                    consecutive_casts = (last_casts or 0) + 1
                
                # Сбросим ли счетчик если сменил локацию и уже сделал 10 забросов там
                if location_changed and consecutive_casts >= 10:
                    cursor.execute('''
                        UPDATE players 
                        SET last_fishing_location = ?, 
                            consecutive_location_casts = 1,
                    ...
                    ''', (location, now, user_id))
                    conn.commit()
                    return location_changed, 1, False
                
                # Обновляем счетчик
                cursor.execute('''
                    UPDATE players 
                    SET last_fishing_location = ?, 
                        consecutive_location_casts = ?,
                        last_cast_timestamp = ?
                    WHERE user_id = ?
                ''', (location, consecutive_casts, now, user_id))
                conn.commit()
                
                # Для локации, где 30+ забросов, показываем предупреждение на 30-м забросе
                show_warning = (consecutive_casts == 30)
                
                return location_changed, consecutive_casts, show_warning
        except Exception as e:
            logger.exception(f'update_population_state failed: {e}')
            return False, 1, False
    
    def get_population_penalty(self, user_id: int) -> float:
        """
        Получить штраф на вероятность (в процентах).
        - 30 забросов: -5%
        - 40 забросов: -8%
        - 50 забросов: -11%
        - 60+ забросов: -15%
        ...
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT consecutive_location_casts
                    FROM players WHERE user_id = ?
                ''', (user_id,))
                row = cursor.fetchone()
            ...
            logger.exception(f'get_population_penalty failed: {e}')
            return 0.0
    ...
    def get_consecutive_casts(self, user_id: int) -> int:
        ...
        try:
            ...
            logger.exception(f'get_consecutive_casts failed: {e}')
            return 0
