...
from fish_activity import filter_fish_by_time, get_activity_for_fish_name
from achievements import (
    ACHIEVEMENTS,
    ACHIEVEMENT_BY_ID,
    highest_reachable_tier,
    tier_description,
    tier_title,
)
...
class Database:
    ...
    def update_player_sale_stats(self, user_id: int, weight: float, price: int, fish_count: int = 1):
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                WHERE user_id = ?
                ''',
                (float(weight), int(price), int(fish_count), int(user_id))
            )
            conn.commit()

        self.evaluate_achievements(int(user_id))

    def _collect_achievement_stats(self, user_id: int) -> Dict[str, float]:
        stats = self.get_player_stats(int(user_id), 0)
        level = 0
        try:
            ...
            level = 0
        return {
            "unique_fish": float(stats.get("unique_fish") or 0),
            "total_fish": float(stats.get("total_fish") or 0),
            "total_weight": float(stats.get("total_weight") or 0),
            "sold_fish_count": float(stats.get("sold_fish_count") or 0),
            "total_coins_earned": float(stats.get("total_coins_earned") or 0),
            "total_trash_caught": float(stats.get("total_trash_caught") or 0),
            "biggest_weight": float(stats.get("biggest_weight") or 0),
            "level": float(level),
        }
    ...
    def evaluate_achievements(
        ...
        """Проверить и выдать новые уровни достижений."""
        uid = int(user_id)
        ...
        with self._connect() as conn:
            ...
            for ach in ACHIEVEMENTS:
                ach_id = ach["id"]
                stat_key = ach["stat"]
                current_value = float(stat_values.get(stat_key) or 0)
                target_tier = highest_reachable_tier(ach_id, current_value)
                if target_tier <= 0:
                    continue
                ...
                if target_tier <= existing_max:
                    continue

                for tier_num in range(existing_max + 1, target_tier + 1):
                    cursor.execute(
                        '''
                        INSERT INTO player_achievements (user_id, achievement_id, tier)
                        VALUES (?, ?, ?)
                    ...
                    unlock = {
                        ...
                        "tier": tier_num,
                        "title": tier_title(ach_id, tier_num),
                        "description": tier_description(ach_id, tier_num),
                        "icon": ach.get("icon", "🏆"),
        ...
        if newly_unlocked:
            bucket = self._achievement_notifications.setdefault(uid, [])
            bucket.extend(newly_unlocked)
        return newly_unlocked

    def drain_achievement_notifications(self, user_id: int) -> List[Dict[str, Any]]:
        uid = int(user_id)
        items = self._achievement_notifications.pop(uid, [])
        return list(items)

    def get_achievement_holder_counts(self) -> Dict[str, Dict[int, int]]:
        """Сколько игроков имеют каждый уровень достижения."""
        counts: Dict[str, Dict[int, int]] = {}
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                '''
                SELECT achievement_id, tier, COUNT(DISTINCT user_id)
                FROM player_achievements
                GROUP BY achievement_id, tier
                '''
            )
            for ach_id, tier, cnt in cursor.fetchall() or []:
                counts.setdefault(str(ach_id), {})[int(tier)] = int(cnt or 0)
        return counts
    ...
    def get_user_achievement_progress(self, user_id: int) -> Dict[str, Any]:
        uid = int(user_id)
        stat_values = self._collect_achievement_stats(uid)
        holder_counts = self.get_achievement_holder_counts()
        total_players = max(1, self.get_total_players_count())

        unlocked_by_ach: Dict[str, Dict[int, str]] = {}
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                '''
            ...
            for ach_id, tier, unlocked_at in cursor.fetchall() or []:
                ach_key = str(ach_id)
                unlocked_by_ach.setdefault(ach_key, {})[int(tier)] = (
                    unlocked_at.isoformat() if hasattr(unlocked_at, "isoformat") else str(unlocked_at)
                )
        ...
        for ach in ACHIEVEMENTS:
            ...
            current_value = float(stat_values.get(stat_key) or 0)
            tiers = ach["tiers"]
            max_tier = int(tiers[-1]["tier"])
            user_tiers = unlocked_by_ach.get(ach_id, {})
            current_tier = max(user_tiers.keys()) if user_tiers else 0

            if current_tier > 0:
                display_tier = current_tier
                display_desc = tier_description(ach_id, display_tier)
                unlocked_at = user_tiers.get(display_tier)
            else:
                display_tier = 1
                display_desc = tier_description(ach_id, 1)
                unlocked_at = None

            holders = holder_counts.get(ach_id, {}).get(display_tier if current_tier > 0 else 1, 0)
            holder_percent = round((holders / total_players) * 100, 1)

            next_tier = None
            next_threshold = None
            if current_tier < max_tier:
                next_tier_num = current_tier + 1 if current_tier > 0 else 1
                for t in tiers:
                    if int(t["tier"]) == next_tier_num:
            ...
            items.append({
                ...
                "display_title": tier_title(ach_id, display_tier) if current_tier > 0 else ach["title"],
                "description": display_desc,
                "unlocked_at": unlocked_at,
                "is_unlocked": current_tier > 0,
                "holder_percent": holder_percent,
                ...
                "stat_label": ach.get("stat_label", ""),
            })

        earned_count = sum(1 for i in items if i["is_unlocked"])
        return {
            "items": items,
    ...
    def get_sea_god_catch_modifier_percent(self, user_id: int) -> float:
        """Бонус/штраф клева от молитвы Морскому Повелителю."""
        from sea_pray import BLESSING_EFFECT, BLESSING_CATCH_BONUS, WRATH_CATCH_PENALTY, WRATH_EFFECT
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                ''',
                (int(user_id), now, WRATH_EFFECT, BLESSING_EFFECT),
            )
            for row in cursor.fetchall() or []:
                effect_type = str(row[0] or '').strip().lower()
                if effect_type == WRATH_EFFECT:
                    modifier += WRATH_CATCH_PENALTY
                elif effect_type == BLESSING_EFFECT:
                    modifier += BLESSING_CATCH_BONUS
        return float(modifier)

    def get_pray_cooldown_remaining(self, user_id: int) -> Optional[timedelta]:
        """Сколько осталось до следующей молитвы."""
        from sea_pray import PRAY_COOLDOWN_HOURS
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                FROM players
                WHERE user_id = ?
                  AND last_pray_at IS NOT NULL
                ORDER BY last_pray_at DESC
                LIMIT 1
                ''',
                (int(user_id),),
            )
        ...
        if not row or not row[0]:
            return None
        try:
            last_at = row[0] if isinstance(row[0], datetime) else datetime.fromisoformat(str(row[0]))
        except Exception:
            return None
        if last_at.tzinfo is not None:
            last_at = last_at.replace(tzinfo=None)
        cooldown_until = last_at + timedelta(hours=PRAY_COOLDOWN_HOURS)
        remaining = cooldown_until - datetime.utcnow()
        if remaining.total_seconds() <= 0:
            return None
        return remaining

    def mark_pray_used(self, user_id: int) -> None:
        now_iso = datetime.utcnow().isoformat()
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE players SET last_pray_at = ? WHERE user_id = ?',
                (now_iso, int(user_id)),
            )
            conn.commit()

    def add_player_coins_all_profiles(self, user_id: int, amount: int) -> int:
        """Начислить монеты на все профили игрока."""
        ...
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE players SET coins = COALESCE(coins, 0) + ? WHERE user_id = ?',
                (amt, int(user_id)),
            )
            conn.commit()
        return amt
    ...
    def break_active_rod_by_divine_wrath(self, user_id: int, chat_id: int) -> Optional[str]:
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                '''
                UPDATE player_rods
                SET current_durability = 0, recovery_start_time = NULL
    ...
    def grant_paid_boat_divine(self, user_id: int) -> bool:
        """Подарить платную лодку без списания бриллиантов."""
        self._ensure_boat_tables()
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT 1 FROM boats WHERE user_id = ? AND type = 'paid' LIMIT 1",
                (int(user_id),),
            )
            if cursor.fetchone():
                return False
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                VALUES (?, 'paid', ?, ?, ?, ?, ?, 0)
                ''',
                (int(user_id), 'Корабль Аргонавтов', 3, 1500.0, 150, 150),
            )
            cursor.execute(
                "SELECT id FROM boats WHERE user_id = ? AND type = 'paid' ORDER BY id DESC LIMIT 1",
                (int(user_id),),
            )
            row = cursor.fetchone()
            if not row:
            ...
            boat_id = row[0]
            cursor.execute(
                'INSERT INTO boat_members (boat_id, user_id, is_owner) VALUES (?, ?, 1)',
                (boat_id, int(user_id)),
            )
            conn.commit()
        return True
    ...
    def _ensure_project_donations_table(self):
        """Таблица пожертвований проекту (Telegram Stars в личке с ботом)."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute('''
            ...
            ''')
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_project_donations_user
                ON project_donations (user_id, created_at DESC)
            ''')
            conn.commit()
    ...
    def _ensure_extended_gameplay_tables(self):
        ...
        with self._connect() as conn:
            ...
            # Новая универсальная таблица событий на локациях
            cursor.execute('''
            ...
            ''')
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_location_events_active
                ON location_events (location, event_type, is_active, ends_at)
            ''')

            # Таблица для хранения цепочек стайного инстинкта
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS school_chains (
                    id SERIAL PRIMARY KEY,
                    user_id BIGINT NOT NULL,
            ...
            ''')
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_school_chains_user_event
                ON school_chains (user_id, event_id)
            ''')
    ...
    def get_active_location_event(self, location: str, event_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Получить активное событие на локации (любого типа или конкретного)."""
        now = datetime.utcnow()
        with self._connect() as conn:
            cursor = conn.cursor()
            # Сначала деактивируем истекшие события
            cursor.execute(
                '''
            ...
            if event_type:
                cursor.execute(
                    '''
                    SELECT id, event_type, location, started_at, ends_at, is_active, params
            ...
            # Парсим JSON params
            import json
            params_str = event_dict.get('params', '{}')
            try:
                event_dict['params'] = json.loads(params_str) if isinstance(params_str, str) else {}
            except:
                event_dict['params'] = {}
            
            return event_dict

    def get_any_active_location_event(self, event_type: str) -> Optional[Dict[str, Any]]:
        """Проверить, есть ли активное событие данного типа на любой локации."""
        now = datetime.utcnow()
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                ...
                SELECT id, event_type, location, started_at, ends_at, is_active, params
                FROM location_events
                WHERE event_type = ? AND is_active = 1 AND ends_at > ?
                ORDER BY started_at DESC
                LIMIT 1
                ''',
                (event_type, now)
    ...
    def get_all_active_events_on_location(self, location: str) -> List[Dict[str, Any]]:
        """Получить ВСЕ активные события на локации (для отображения)."""
        now = datetime.utcnow()
        with self._connect() as conn:
            cursor = conn.cursor()
            # Сначала деактивируем истекшие события
    ...
    def start_location_event(
        ...
        """Начать новое событие на локации."""
        import json
        now = datetime.utcnow()
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                ...
                INSERT INTO location_events (
                    event_type, location, started_at, ends_at, is_active, params
                )
                VALUES (?, ?, ?, ?, 1, ?)
                RETURNING id, event_type, location, started_at, ends_at, is_active, params
                ''',
                (event_type, location, now, ends_at, params_json)
            )
            row = cursor.fetchone()
            columns = [d[0] for d in cursor.description] if cursor.description else []
            conn.commit()
            
            event_dict = dict(zip(columns, row)) if row else {}
            if event_dict:
                try:
                    event_dict['params'] = json.loads(event_dict.get('params', '{}'))
                except:
                    event_dict['params'] = {}
            
            return event_dict

    def stop_location_event(self, location: str, event_type: Optional[str] = None) -> bool:
        """Остановить событие на локации."""
        with self._connect() as conn:
            cursor = conn.cursor()
            if event_type:
                cursor.execute(
    ...
    def maybe_start_location_event(
        self,
        location: str,
        event_type: str,
        chance: float,
        ...
        """
        Попытаться начать событие на локации с учетом шанса и cooldown.
        
        ПРАВИЛА:
        - Только ОДНО событие (любого типа) может быть активно на одной локации
        - Каждый тип события может быть активен только на ОДНОЙ локации
        
        Args:
            location: название локации
            event_type: тип события
            chance: шанс срабатывания (0.0 - 1.0)
            cooldown_hours: часы cooldown после окончания события
        ...
        if any_active_on_location:
            # Если на этой локации уже есть событие, возвращаем его если это наш тип
            if any_active_on_location['event_type'] == event_type:
                return any_active_on_location
            # Иначе нельзя начинать новое - локация занята
            return None
        ...
        if active_anywhere:
            # Если событие этого типа активно на другой локации - нельзя начинать
            return None
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                '''
                SELECT ends_at FROM location_events 
                WHERE event_type = ? 
    ...
    def get_school_chain(self, user_id: int, event_id: int) -> Dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                '''
                SELECT id, user_id, event_id, location, fish_name, chain_count, last_catch_at
                FROM school_chains
            ...
            row = cursor.fetchone()
            if not row:
                return {'chain_count': 0, 'fish_name': ''}
            
            columns = [d[0] for d in cursor.description]
            return dict(zip(columns, row))

    def update_school_chain(
        self,
        user_id: int,
        event_id: int,
        location: str,
        ...
        """Обновить цепочку стайного инстинкта."""
        now = datetime.utcnow()
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                '''
                INSERT INTO school_chains (user_id, event_id, location, fish_name, chain_count, last_catch_at)
                VALUES (?, ?, ?, ?, ?, ?)
                ...
                ''',
                (int(user_id), int(event_id), location, fish_name, int(chain_count), now)
            )
            conn.commit()

    def reset_school_chain(self, user_id: int, event_id: int) -> None:
        """Сбросить цепочку стайного инстинкта."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                '''
                DELETE FROM school_chains
                WHERE user_id = ? AND event_id = ?
                ''',
                (int(user_id), int(event_id))
            )
    ...
    def __init__(self):
        ...
        self._achievement_notifications: Dict[int, List[Dict[str, Any]]] = {}
    ...
    def init_db(self):
        ...
        with self._connect() as conn:
            ...
            self._ensure_antibot_captcha_table()
            self._ensure_duel_tables()
            self._ensure_project_donations_table()
    ...
    def _run_migrations_inner(self, conn):
        ...
        try:
            ...
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_player_achievements_ach ON player_achievements(achievement_id, tier)'
            )
            conn.commit()
        except Exception:
            try:
                ...
                pass
        ensure_column('locations', 'required_level', 'INTEGER DEFAULT 0')
    ...
    def _fill_default_data(self):
        ...
        with self._connect() as conn:
            ...
            # Добавление локаций
            locations_data = [
                ("Городской пруд", 100, 0, 5, 0),
                ("Река", 150, 0, 10, 0),
                ("Озеро", 200, 0, 15, 0),
                ("Море", 300, 0, 20, 0),
                ("Коралловый риф", 250, 0, 12, 5),
                ("Глубоководный желоб", 200, 0, 8, 8),
                ("Мангровые заросли", 180, 0, 10, 10),
            ]
            ...
            cursor.executemany('''
                INSERT OR IGNORE INTO locations (name, fish_population, current_players, max_players, required_level)
                VALUES (?, ?, ?, ?, ?)
            ''', locations_data)
            ...
            base_baits_data = [
                ...
                ("Дафния", 35, 1, "Все"),
                ("Детрит", 20, 0, "Все"),
                ("Зоопланктон", 35, 1, "Все"),
                ("Коралловые полипы", 40, 2, "Все"),
                ("Коралловый песок", 20, 0, "Все"),
                ("Краб", 70, 4, "Все"),
                ("Кровь", 40, 2, "Все"),
                ("Крупная рыба", 120, 8, "Все"),
                ("Крупный планктон", 45, 2, "Все"),
                ("Личинки", 30, 1, "Все"),
                ("Мелкая рыба", 60, 3, "Все"),
                ("Мелкие беспозвоночные", 40, 2, "Все"),
                ("Мелкие животные", 45, 2, "Все"),
                ("Мелкие рачки", 40, 2, "Все"),
                ("Моллюски", 60, 4, "Все"),
                ("Морской еж", 80, 5, "Все"),
                ("Мшанки", 35, 1, "Все"),
                ("Насекомые", 30, 1, "Все"),
            ...
            fish_data = [
                ...
                # ===== КОРАЛЛОВЫЙ РИФ =====
                ("Рыба-клоун", "Обычная", 0.05, 0.3, 8, 15, 25, "Коралловый риф", "Все", "Креветка,Мякиш хлеба,Морской червь,Опарыш", 5, None),
                ("Рыба-хирург", "Обычная", 0.1, 0.5, 12, 25, 30, "Коралловый риф", "Все", "Креветка,Морской червь,Опарыш,Мотыль", 6, None),
                ("Морской конёк", "Редкая", 0.01, 0.05, 5, 15, 50, "Коралловый риф", "Все", "Креветка,Мотыль,Опарыш", 4, None),
                ("Краб-отшельник", "Обычная", 0.02, 0.1, 3, 10, 20, "Коралловый риф", "Все", "Морской червь,Кусочки рыбы,Креветка", 4, None),
                ("Осьминог рифовый", "Редкая", 0.5, 3.0, 20, 60, 80, "Коралловый риф", "Все", "Кусочки рыбы,Креветка,Живец,Кальмар", 12, None),
                ("Актиния", "Обычная", 0.1, 0.5, 5, 20, 15, "Коралловый риф", "Все", "Морской червь,Креветка", 5, None),
                ("Мурена-зебра", "Легендарная", 2.0, 15.0, 50, 150, 300, "Коралловый риф", "Все", "Живец,Кусочки рыбы,Кальмар,Кусок мяса", 30, None),
                ("Спинорог", "Редкая", 0.3, 2.0, 15, 40, 60, "Коралловый риф", "Все", "Креветка,Кусочки рыбы,Морской червь,Моллюск", 10, None),

                # ===== ГЛУБОКОВОДНЫЙ ЖЕЛОБ =====
                ("Гигантский изопод", "Редкая", 0.5, 2.0, 15, 40, 100, "Глубоководный желоб", "Все", "Кусочки рыбы,Морской червь,Креветка", 10, None),
                ("Рыба-удильщик", "Легендарная", 3.0, 20.0, 30, 100, 400, "Глубоководный желоб", "Все", "Живец,Кусочки рыбы,Кальмар,Крупный живец", 35, None),
                ("Вампировый кальмар", "Мифическая", 1.0, 5.0, 20, 50, 500, "Глубоководный желоб", "Все", "Кусочки рыбы,Креветка,Морской червь", 20, None),
                ("Медуза-корнерот", "Редкая", 2.0, 10.0, 30, 80, 150, "Глубоководный желоб", "Все", "Креветка,Морской червь", 18, None),
                ("Глубоководный удильщик (самка)", "Мифическая", 5.0, 30.0, 40, 120, 800, "Глубоководный желоб", "Все", "Крупный живец,Кусочки рыбы,Кальмар,Живец", 45, None),
                ("Осьминог Дамбо", "Легендарная", 1.0, 8.0, 20, 60, 350, "Глубоководный желоб", "Все", "Кусочки рыбы,Креветка,Кальмар", 25, None),
                ("Стеклянный кальмар", "Редкая", 0.3, 2.0, 10, 40, 120, "Глубоководный желоб", "Все", "Кусочки рыбы,Морской червь,Креветка", 12, None),

                # ===== МАНГРОВЫЕ ЗАРОСЛИ =====
                ("Илистый прыгун", "Обычная", 0.05, 0.3, 8, 20, 25, "Мангровые заросли", "Все", "Морской червь,Креветка,Опарыш", 5, None),
                ("Мангровый краб", "Обычная", 0.1, 0.8, 10, 25, 30, "Мангровые заросли", "Все", "Кусочки рыбы,Морской червь,Креветка", 6, None),
                ("Рыба-архитектор", "Редкая", 0.2, 1.5, 12, 35, 80, "Мангровые заросли", "Все", "Креветка,Морской червь,Живец,Кусочки рыбы", 10, None),
                ("Моллюск-древоточец", "Обычная", 0.05, 0.4, 5, 15, 20, "Мангровые заросли", "Все", "Морской червь,Креветка", 5, None),
                ("Крылатка-зебра", "Легендарная", 1.0, 5.0, 20, 50, 250, "Мангровые заросли", "Все", "Живец,Кусочки рыбы,Креветка,Кальмар", 18, None),
                ("Морская звезда-подсолнух", "Редкая", 0.5, 3.0, 15, 60, 100, "Мангровые заросли", "Все", "Моллюск,Морской червь,Кусочки рыбы", 12, None),
                ("Личинка угря (лептоцефал)", "Редкая", 0.01, 0.1, 5, 15, 60, "Мангровые заросли", "Все", "Мотыль,Опарыш,Морской червь", 5, None),

                # ===== ДОПОЛНЕНИЯ ДЛЯ СУЩЕСТВУЮЩИХ ЛОКАЦИЙ =====
                # Пруд
                ("Американский рак", "Редкая", 0.1, 0.8, 10, 25, 40, "Городской пруд", "Весна,Лето,Осень", "Кусочки рыбы,Черви,Мотыль,Опарыш", 6, None),
                ("Лягушка-бык", "Редкая", 0.2, 1.0, 10, 20, 45, "Городской пруд", "Весна,Лето,Осень", "Черви,Муха,Опарыш,Мотыль", 8, None),
                # Река
                ("Речная минога", "Редкая", 0.05, 0.4, 15, 40, 50, "Река", "Все", "Черви,Мотыль,Личинка,Опарыш", 8, None),
                ("Камбала речная", "Редкая", 0.3, 2.0, 20, 45, 55, "Река", "Весна,Лето,Осень", "Черви,Кусочки рыбы,Мотыль,Выползок", 10, None),
                # Озеро
                ("Озёрная медуза", "Редкая", 0.1, 0.5, 10, 30, 40, "Озеро", "Лето", "Мотыль,Опарыш", 6, None),
                ("Беззубка", "Обычная", 0.05, 0.3, 5, 15, 15, "Озеро", "Все", "Мотыль,Опарыш,Черви", 5, None),
                # Море
                ("Кальмар-стрела", "Редкая", 0.5, 3.0, 20, 60, 70, "Море", "Все", "Кусочки рыбы,Креветка,Живец,Морской червь", 12, None),
                ("Морской ёж", "Обычная", 0.1, 0.6, 5, 15, 20, "Море", "Все", "Морской червь,Креветка", 5, None),
                ("Рыба-клоун", "Обычная", 0.01, 0.03, 5.0, 10.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Мотыль,Опарыш,Тесто", 6, None),
                ("Рыба-хирург", "Редкая", 0.2, 1.5, 15.0, 40.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Кукуруза,Горох,Черви,Хлеб", 6, None),
                ("Морской конёк", "Редкая", 0.005, 0.02, 5.0, 15.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Мотыль,Опарыш,Тесто", 6, None),
                ("Краб-отшельник", "Обычная", 0.01, 0.5, 3.0, 15.0, 10, "Коралловый риф", "Круглый Год", "Черви,Кусочки рыбы,Сало", 6, None),
                ("Осьминог рифовый", "Редкая", 0.5, 5.0, 30.0, 100.0, 25, "Коралловый риф", "Круглый Год", "Живец,Блесна,Воблер", 12, None),
                ("Актиния", "Обычная", 0.1, 2.0, 10.0, 50.0, 10, "Коралловый риф", "Круглый Год", "Мотыль,Опарыш,Тесто", 6, None),
                ("Мурена-зебра", "Редкая", 2.0, 15.0, 60.0, 200.0, 60, "Коралловый риф", "Круглый Год", "Кусочки рыбы", 20, None),
                ("Спинорог", "Редкая", 0.5, 3.0, 20.0, 50.0, 25, "Коралловый риф", "Круглый Год", "Моллюск,Кукуруза,Горох", 6, None),
                ("Гигантский изопод", "Редкая", 0.5, 2.0, 15.0, 40.0, 25, "Глубоководный желоб", "Круглый Год", "Кусочки рыбы,Черви,Мотыль", 6, None),
                ("Рыба-удильщик", "Мифическая", 5.0, 30.0, 50.0, 150.0, 300, "Глубоководный желоб", "Круглый Год", "Живец,Кусочки рыбы", 30, None),
                ("Вампировый кальмар", "Редкая", 0.1, 0.5, 20.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Мотыль,Опарыш,Тесто", 6, None),
                ("Медуза-корнерот", "Обычная", 5.0, 20.0, 50.0, 200.0, 10, "Глубоководный желоб", "Круглый Год", "Мотыль,Опарыш,Тесто", 30, None),
                ("Глубоководный удильщик (самка)", "Мифическая", 1.0, 15.0, 20.0, 120.0, 300, "Глубоководный желоб", "Круглый Год", "Кусочки рыбы", 20, None),
                ("Осьминог Дамбо", "Редкая", 1.0, 5.0, 30.0, 80.0, 25, "Глубоководный желоб", "Круглый Год", "Черви,Мотыль,Опарыш", 12, None),
                ("Стеклянный кальмар", "Редкая", 0.05, 0.3, 10.0, 25.0, 25, "Глубоководный желоб", "Круглый Год", "Опарыш", 6, None),
                ("Илистый прыгун", "Обычная", 0.01, 0.05, 5.0, 15.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Черви,Мотыль,Опарыш", 6, None),
                ("Мангровый краб", "Обычная", 0.1, 0.5, 10.0, 20.0, 10, "Мангровые заросли", "Круглый Год", "Черви,Кусочки рыбы,Сало", 6, None),
                ("Рыба-архитектор", "Редкая", 0.02, 0.1, 8.0, 18.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Хлеб,Манка,Черви,Опарыш,Тесто,Кукуруза", 6, None),
                ("Моллюск-древоточец", "Обычная", 0.05, 0.5, 10.0, 30.0, 10, "Мангровые заросли", "Круглый Год", "Мотыль,Опарыш,Тесто", 6, None),
                ("Крылатка-зебра", "Редкая", 0.3, 1.5, 20.0, 40.0, 25, "Мангровые заросли", "Круглый Год", "Живец,Блесна,Воблер", 6, None),
                ("Морская звезда-подсолнух", "Редкая", 0.5, 3.0, 40.0, 80.0, 25, "Мангровые заросли", "Круглый Год", "Моллюск,Кусочки рыбы,Черви", 6, None),
                ("Личинка угря (лептоцефал)", "Обычная", 0.001, 0.005, 5.0, 15.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Мотыль,Опарыш,Тесто", 6, None),
                ("Рыба-ангел императорский", "Редкая", 0.2, 1.5, 20.0, 40.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Губки,Мшанки,Водоросли,Мотыль", 6, None),
                ("Рыба-ангел королевский", "Редкая", 0.3, 2.0, 25.0, 45.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Губки,Мшанки,Водоросли,Мотыль", 6, None),
                ("Рыба-бабочка длиннорылая", "Обычная", 0.05, 0.15, 10.0, 20.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Коралловые полипы,Черви,Креветка", 6, None),
                ("Рыба-бабочка пятнистая", "Обычная", 0.03, 0.1, 8.0, 15.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Полипы,Черви,Мотыль", 6, None),
                ("Рыба-попугай зеленый", "Редкая", 1.0, 8.0, 40.0, 90.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Коралловый песок,Креветка", 12, None),
                ("Рыба-попугай синий", "Редкая", 0.5, 5.0, 30.0, 70.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Мелкие беспозвоночные", 12, None),
                ("Рыба-хирург желтый", "Обычная", 0.1, 0.5, 12.0, 25.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Хлеб,Салат", 6, None),
                ("Спинорог клоун", "Редкая", 0.2, 1.0, 20.0, 40.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Моллюск,Ракушка,Креветка", 6, None),
                ("Спинорог тигровый", "Редкая", 0.3, 1.5, 25.0, 50.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Моллюск,Краб,Креветка", 6, None),
                ("Кузовок рогатый", "Обычная", 0.1, 0.4, 10.0, 20.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Черви,Мотыль", 6, None),
                ("Кузовок пятнистый", "Обычная", 0.1, 0.3, 8.0, 15.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Мелкие рачки", 6, None),
                ("Рыба-собака полосатая", "Редкая", 0.2, 1.0, 15.0, 30.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Коралловые полипы,Водоросли,Креветка", 6, None),
                ("Рыба-собака пятнистая", "Редкая", 0.3, 1.2, 20.0, 40.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Моллюски,Черви", 6, None),
                ("Групер красный", "Редкая", 5.0, 30.0, 60.0, 120.0, 60, "Коралловый риф", "Круглый Год", "Рыба,Краб,Креветка", 30, None),
                ("Групер тигровый", "Редкая", 10.0, 50.0, 80.0, 150.0, 60, "Коралловый риф", "Круглый Год", "Рыба,Кальмар,Краб", 50, None),
                ("Групер гигантский рифовый", "Мифическая", 50.0, 400.0, 150.0, 270.0, 800, "Коралловый риф", "Круглый Год", "Крупная рыба,Скат,Краб", 80, None),
                ("Мурена пятнистая", "Редкая", 1.0, 10.0, 50.0, 200.0, 60, "Коралловый риф", "Круглый Год", "Рыба,Кальмар,Краб", 20, None),
                ("Мурена гигантская", "Мифическая", 10.0, 40.0, 150.0, 300.0, 300, "Коралловый риф", "Круглый Год", "Крупная рыба,Осьминог", 30, None),
                ("Рыба-камень", "Мифическая", 0.2, 1.0, 15.0, 30.0, 300, "Коралловый риф", "Круглый Год", "Мелкая рыба,Креветка", 6, None),
                ("Рыба-лев", "Мифическая", 0.3, 1.5, 20.0, 40.0, 300, "Коралловый риф", "Круглый Год", "Мелкая рыба,Креветка", 6, None),
                ("Скат-манта рифовый", "Мифическая", 200.0, 1000.0, 300.0, 500.0, 800, "Коралловый риф", "Весна,Лето,Осень", "Планктон,Рачки", 80, None),
                ("Скат-орляк", "Редкая", 10.0, 50.0, 100.0, 200.0, 60, "Коралловый риф", "Весна,Лето,Осень", "Моллюск,Краб,Рак", 50, None),
                ("Осьминог карибский", "Обычная", 0.5, 2.0, 30.0, 60.0, 10, "Коралловый риф", "Круглый Год", "Краб,Креветка,Мелкая рыба", 6, None),
                ("Осьминог обыкновенный рифовый", "Редкая", 1.0, 5.0, 40.0, 100.0, 25, "Коралловый риф", "Круглый Год", "Краб,Рак,Моллюск", 12, None),
                ("Кальмар карибский", "Обычная", 0.2, 1.0, 20.0, 40.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Мелкая рыба,Креветка", 6, None),
                ("Креветка-чистильщик", "Обычная", 0.005, 0.02, 2.0, 5.0, 10, "Коралловый риф", "Круглый Год", "Паразиты,Мотыль,Остатки пищи", 6, None),
                ("Креветка-боксер", "Обычная", 0.01, 0.03, 3.0, 6.0, 10, "Коралловый риф", "Круглый Год", "Рачки,Черви,Падаль", 6, None),
                ("Краб-паук", "Обычная", 0.1, 0.5, 10.0, 20.0, 10, "Коралловый риф", "Круглый Год", "Падаль,Водоросли,Мелкие животные", 6, None),
                ("Краб-вампир", "Редкая", 0.05, 0.2, 5.0, 10.0, 25, "Коралловый риф", "Круглый Год", "Падаль,Водоросли", 6, None),
                ("Морская звезда терновый венец", "Редкая", 1.0, 5.0, 30.0, 60.0, 25, "Коралловый риф", "Круглый Год", "Коралловые полипы", 12, None),
                ("Морская звезда синяя", "Обычная", 0.2, 1.0, 15.0, 30.0, 10, "Коралловый риф", "Круглый Год", "Моллюск,Детрит", 6, None),
                ("Морской еж черный", "Обычная", 0.1, 0.5, 10.0, 15.0, 10, "Коралловый риф", "Круглый Год", "Водоросли,Детрит", 6, None),
                ("Морской еж красный", "Обычная", 0.1, 0.4, 8.0, 12.0, 10, "Коралловый риф", "Круглый Год", "Водоросли,Обрастания", 6, None),
                ("Актиния ковровая", "Обычная", 0.5, 2.0, 20.0, 50.0, 10, "Коралловый риф", "Круглый Год", "Планктон,Мотыль", 6, None),
                ("Коралл мозговидный", "Обычная", 0.01, 0.1, 20.0, 100.0, 10, "Коралловый риф", "Круглый Год", "Свет,Планктон", 6, None),
                ("Коралл ветвистый", "Обычная", 0.01, 0.1, 30.0, 150.0, 10, "Коралловый риф", "Круглый Год", "Свет,Планктон", 6, None),
                ("Губка ваза", "Обычная", 1.0, 5.0, 30.0, 60.0, 10, "Коралловый риф", "Круглый Год", "Бактерии,Детрит", 12, None),
                ("Асцидия", "Обычная", 0.05, 0.3, 5.0, 15.0, 10, "Коралловый риф", "Круглый Год", "Планктон,Бактерии", 6, None),
                ("Рыба-каранкс рифовый", "Обычная", 1.0, 10.0, 40.0, 100.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Мелкая рыба,Креветка", 20, None),
                ("Рыба-луциан красный", "Редкая", 1.0, 25.0, 40.0, 90.0, 60, "Коралловый риф", "Весна,Лето,Осень", "Рыба,Кальмар,Креветка", 30, None),
                ("Рыба-снук", "Редкая", 1.0, 8.0, 40.0, 100.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Креветка,Краб,Мелкая рыба", 12, None),
                ("Рыба-помацентр желтохвостый", "Обычная", 0.01, 0.03, 4.0, 8.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Планктон", 6, None),
                ("Рыба-абудефдуф зеленый", "Обычная", 0.005, 0.02, 3.0, 6.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Планктон,Водоросли", 6, None),
                ("Рыба-дасциллус синий", "Обычная", 0.01, 0.04, 5.0, 9.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Зоопланктон", 6, None),
                ("Рыба-зеленушка рифовая", "Обычная", 0.05, 0.3, 10.0, 20.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Рачки,Черви,Мотыль", 6, None),
                ("Рыба-губан", "Редкая", 1.0, 10.0, 30.0, 90.0, 60, "Коралловый риф", "Весна,Лето,Осень", "Моллюск,Краб,Морской еж", 20, None),
                ("Рыба-попугай двуцветный", "Обычная", 0.5, 3.0, 25.0, 50.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Коралловый песок", 6, None),
                ("Рыба-хирург парусный", "Редкая", 0.5, 2.0, 25.0, 45.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Водоросли,Салат,Хлеб", 6, None),
                ("Рыба-бабочка оранжевая", "Обычная", 0.03, 0.1, 8.0, 14.0, 10, "Коралловый риф", "Весна,Лето,Осень", "Коралловые полипы,Черви", 6, None),
                ("Рыба-ангел французский", "Редкая", 0.3, 2.0, 20.0, 40.0, 25, "Коралловый риф", "Весна,Лето,Осень", "Губки,Водоросли,Мотыль", 6, None),
                ("Рыба-топорик серебристый", "Обычная", 0.01, 0.05, 5.0, 10.0, 10, "Глубоководный желоб", "Круглый Год", "Зоопланктон,Мелкие рачки", 6, None),
                ("Рыба-топорик темный", "Обычная", 0.01, 0.04, 4.0, 8.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Рачки", 6, None),
                ("Рыба-гадюка обыкновенная", "Редкая", 0.1, 0.3, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Креветка", 6, None),
                ("Рыба-мешкорот пеликановая", "Редкая", 0.5, 2.0, 30.0, 80.0, 25, "Глубоководный желоб", "Круглый Год", "Крупный планктон,Мелкая рыба", 6, None),
                ("Рыба-мешкорот черный", "Редкая", 0.5, 1.5, 20.0, 60.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 6, None),
                ("Батизавр", "Редкая", 1.0, 3.0, 30.0, 70.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Креветка", 6, None),
                ("Хаулиод тихоокеанский", "Редкая", 0.05, 0.2, 10.0, 20.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Рачки", 6, None),
                ("Рыба-иглорот", "Редкая", 0.05, 0.15, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Планктон,Мелкие рачки", 6, None),
                ("Гидролик (рыба-вампир)", "Редкая", 0.01, 0.03, 5.0, 10.0, 25, "Глубоководный желоб", "Круглый Год", "Кровь,Чешуя,Паразиты", 6, None),
                ("Рыба-дракон глубоководная", "Редкая", 0.1, 0.4, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Рачки", 6, None),
                ("Длиннохвостый угорь", "Обычная", 0.5, 2.0, 50.0, 150.0, 10, "Глубоководный желоб", "Круглый Год", "Черви,Моллюск,Рачки", 6, None),
                ("Глубоководный угорь серый", "Редкая", 1.0, 5.0, 60.0, 180.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 12, None),
                ("Рыба-фонареглаз", "Обычная", 0.01, 0.05, 5.0, 10.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Рачки", 6, None),
                ("Рыба-светлячок (миктоф)", "Обычная", 0.005, 0.02, 3.0, 7.0, 10, "Глубоководный желоб", "Круглый Год", "Зоопланктон,Рачки", 6, None),
                ("Глубоководная зубатка", "Редкая", 5.0, 20.0, 60.0, 120.0, 60, "Глубоководный желоб", "Круглый Год", "Моллюск,Краб,Рыба", 30, None),
                ("Рыба-пеликан", "Редкая", 0.5, 2.0, 30.0, 100.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 6, None),
                ("Осьминог-вампир глубоководный", "Редкая", 0.2, 1.0, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкие рачки,Детрит", 6, None),
                ("Глубоководный кальмар стеклянный", "Редкая", 0.05, 0.3, 10.0, 25.0, 25, "Глубоководный желоб", "Круглый Год", "Креветка,Мелкая рыба", 6, None),
                ("Глубоководная медуза ушастая", "Обычная", 0.1, 0.5, 10.0, 30.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Мелкие рачки", 6, None),
                ("Медуза-гребневик", "Обычная", 0.05, 0.3, 5.0, 15.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Личинки", 6, None),
                ("Глубоководный краб-паук", "Обычная", 0.5, 2.0, 20.0, 50.0, 10, "Глубоководный желоб", "Круглый Год", "Падаль,Детрит,Черви", 6, None),
                ("Глубоководная креветка", "Обычная", 0.005, 0.02, 2.0, 5.0, 10, "Глубоководный желоб", "Круглый Год", "Детрит,Планктон", 6, None),
                ("Рыба-бритва", "Редкая", 0.01, 0.04, 6.0, 12.0, 25, "Глубоководный желоб", "Круглый Год", "Планктон,Рачки", 6, None),
                ("Рыба-труба глубоководная", "Редкая", 0.02, 0.08, 10.0, 20.0, 25, "Глубоководный желоб", "Круглый Год", "Рачки,Черви", 6, None),
                ("Рыба-саблезуб", "Редкая", 0.05, 0.15, 8.0, 15.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Рачки", 6, None),
                ("Рыба-киллер (гидра)", "Редкая", 0.1, 0.3, 12.0, 25.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Креветка", 6, None),
                ("Акула-гоблин", "Легендарная", 300.0, 600.0, 200.0, 400.0, 400, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 80, None),
                ("Акула-домовой", "Легендарная", 150.0, 400.0, 150.0, 300.0, 400, "Глубоководный желоб", "Круглый Год", "Рыба,Краб,Кальмар", 80, None),
                ("Акула-пилонос", "Редкая", 100.0, 300.0, 100.0, 200.0, 60, "Глубоководный желоб", "Круглый Год", "Рыба,Краб,Моллюск", 80, None),
                ("Скат-глубоководный", "Редкая", 2.0, 10.0, 50.0, 100.0, 60, "Глубоководный желоб", "Круглый Год", "Черви,Моллюск,Рачки", 20, None),
                ("Рыба-зебра глубоководная", "Обычная", 0.02, 0.08, 5.0, 12.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Рачки", 6, None),
                ("Рыба-летучая мышь", "Редкая", 0.2, 1.0, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Детрит,Черви,Рачки", 6, None),
                ("Удильщик иглоносый", "Редкая", 0.5, 2.0, 20.0, 50.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Креветка", 6, None),
                ("Рыба-свечение (фотоблефар)", "Редкая", 0.01, 0.03, 4.0, 8.0, 25, "Глубоководный желоб", "Круглый Год", "Планктон,Мелкая рыба", 6, None),
                ("Рыба-светящаяся (линтофитес)", "Редкая", 0.005, 0.02, 3.0, 6.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкие рачки,Черви", 6, None),
                ...
                ("Рыба-топорик большой", "Обычная", 0.02, 0.08, 6.0, 12.0, 10, "Глубоководный желоб", "Круглый Год", "Планктон,Рачки", 6, None),
                ("Рыба-нож глубоководная", "Редкая", 0.1, 0.5, 15.0, 30.0, 25, "Глубоководный желоб", "Круглый Год", "Мелкая рыба,Рачки", 6, None),
                ("Рыба-угорь пелагический", "Редкая", 0.5, 2.0, 40.0, 120.0, 25, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 6, None),
                ("Рыба-меч глубоководная", "Легендарная", 200.0, 500.0, 100.0, 300.0, 400, "Глубоководный желоб", "Круглый Год", "Рыба,Кальмар", 80, None),
                ("Рыба-гренадер", "Обычная", 0.5, 3.0, 30.0, 70.0, 10, "Глубоководный желоб", "Круглый Год", "Черви,Рачки,Моллюск", 6, None),
                ("Рыба-макрурус", "Редкая", 0.5, 2.0, 25.0, 60.0, 25, "Глубоководный желоб", "Круглый Год", "Рачки,Черви", 6, None),
                ("Рыба-слизь (липарис)", "Обычная", 0.01, 0.1, 5.0, 15.0, 10, "Глубоководный желоб", "Круглый Год", "Детрит,Рачки", 6, None),
                ("Рыба-бленни глубоководная", "Мифическая", 0.02, 0.06, 4.0, 10.0, 300, "Глубоководный желоб", "Круглый Год", "Водоросли,Рачки,Черви", 6, None),
                ("Мангровая рыба-зеленушка", "Обычная", 0.05, 0.3, 10.0, 20.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Рачки,Черви,Водоросли", 6, None),
                ("Мангровая рыба-луциан", "Редкая", 1.0, 25.0, 30.0, 90.0, 60, "Мангровые заросли", "Весна,Лето,Осень", "Рыба,Креветка,Краб", 30, None),
                ("Мангровая рыба-снук", "Редкая", 1.0, 10.0, 40.0, 100.0, 60, "Мангровые заросли", "Весна,Лето,Осень", "Креветка,Краб,Мелкая рыба", 20, None),
                ("Мангровая кефаль", "Обычная", 0.2, 2.0, 20.0, 50.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Водоросли,Детрит,Черви", 6, None),
                ("Мангровый бычок", "Обычная", 0.01, 0.05, 5.0, 10.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Черви,Рачки,Мотыль", 6, None),
                ("Бычок-прыгун", "Обычная", 0.01, 0.03, 5.0, 8.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Насекомые,Черви,Рачки", 6, None),
                ("Мангровая рыба-иглобрюх", "Редкая", 0.05, 0.2, 8.0, 15.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Моллюск,Черви,Водоросли", 6, None),
                ("Мангровая креветка", "Обычная", 0.005, 0.02, 2.0, 5.0, 10, "Мангровые заросли", "Круглый Год", "Детрит,Водоросли", 6, None),
                ("Мангровый кальмар", "Редкая", 0.05, 0.2, 10.0, 20.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Мелкая рыба,Креветка", 6, None),
                ("Мангровый осьминог", "Редкая", 0.5, 3.0, 30.0, 80.0, 25, "Мангровые заросли", "Круглый Год", "Краб,Креветка,Моллюск", 6, None),
                ("Мангровая актиния", "Обычная", 0.01, 0.05, 2.0, 10.0, 10, "Мангровые заросли", "Круглый Год", "Планктон,Мотыль", 6, None),
                ("Мангровая губка", "Обычная", 0.01, 0.1, 5.0, 20.0, 10, "Мангровые заросли", "Круглый Год", "Бактерии,Детрит", 6, None),
                ("Морской огурец (голотурия)", "Обычная", 0.1, 0.5, 10.0, 30.0, 10, "Мангровые заросли", "Круглый Год", "Детрит,Органические частицы", 6, None),
                ("Морская звезда пятнистая", "Обычная", 0.05, 0.3, 10.0, 20.0, 10, "Мангровые заросли", "Круглый Год", "Моллюск,Детрит", 6, None),
                ("Морской еж диадема", "Обычная", 0.1, 0.5, 5.0, 15.0, 10, "Мангровые заросли", "Круглый Год", "Водоросли,Обрастания", 6, None),
                ("Моллюск-устрица мангровая", "Обычная", 0.05, 0.3, 5.0, 15.0, 10, "Мангровые заросли", "Круглый Год", "Планктон,Частицы", 6, None),
                ("Моллюск-мидия мангровая", "Обычная", 0.02, 0.1, 3.0, 8.0, 10, "Мангровые заросли", "Круглый Год", "Планктон,Детрит", 6, None),
                ("Рак-отшельник мангровый", "Обычная", 0.01, 0.05, 2.0, 6.0, 10, "Мангровые заросли", "Круглый Год", "Падаль,Водоросли", 6, None),
                ("Краб-скрипач", "Обычная", 0.01, 0.05, 2.0, 5.0, 10, "Мангровые заросли", "Круглый Год", "Детрит,Водоросли,Черви", 6, None),
                ("Краб-плавунец", "Обычная", 0.02, 0.1, 3.0, 8.0, 10, "Мангровые заросли", "Круглый Год", "Падаль,Водоросли,Мелкие животные", 6, None),
                ("Краб-призрак", "Обычная", 0.01, 0.03, 2.0, 4.0, 10, "Мангровые заросли", "Круглый Год", "Детрит,Черви,Насекомые", 6, None),
                ("Рыба-жаба мангровая", "Редкая", 0.1, 0.5, 15.0, 25.0, 25, "Мангровые заросли", "Круглый Год", "Рыба,Креветка,Черви", 6, None),
                ("Рыба-собака мангровая", "Редкая", 0.1, 0.4, 10.0, 20.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Моллюск,Черви,Водоросли", 6, None),
                ("Мангровая рыба-попугай", "Редкая", 0.3, 2.0, 20.0, 40.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Водоросли,Детрит", 6, None),
                ("Мангровая рыба-хирург", "Редкая", 0.2, 1.0, 15.0, 30.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Водоросли,Хлеб", 6, None),
                ("Мангровая рыба-ангел", "Редкая", 0.05, 0.2, 10.0, 18.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Водоросли,Губки,Мотыль", 6, None),
                ("Мангровая рыба-бабочка", "Редкая", 0.02, 0.08, 8.0, 14.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Черви,Рачки,Полипы", 6, None),
                ("Мангровая рыба-каранкс", "Редкая", 1.0, 40.0, 40.0, 120.0, 60, "Мангровые заросли", "Весна,Лето,Осень", "Рыба,Креветка,Кальмар", 30, None),
                ("Тарпон атлантический", "Мифическая", 10.0, 200.0, 100.0, 250.0, 800, "Мангровые заросли", "Весна,Лето,Осень", "Рыба,Креветка,Краб", 65, None),
                ("Мангровая рыба-солнечник", "Обычная", 0.01, 0.05, 5.0, 12.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Планктон,Рачки", 6, None),
                ("Мангровая рыба-вьюн", "Обычная", 0.01, 0.03, 5.0, 10.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Черви,Мотыль,Рачки", 6, None),
                ("Мангровый угорь", "Редкая", 0.5, 5.0, 40.0, 120.0, 25, "Мангровые заросли", "Круглый Год", "Рыба,Краб,Черви", 12, None),
                ("Мангровая мурена", "Редкая", 0.5, 2.0, 30.0, 80.0, 25, "Мангровые заросли", "Круглый Год", "Рыба,Кальмар,Краб", 6, None),
                ("Мангровая барракуда", "Легендарная", 5.0, 80.0, 60.0, 180.0, 150, "Мангровые заросли", "Весна,Лето,Осень", "Рыба,Креветка", 50, None),
                ("Мангровая крылатка", "Редкая", 0.2, 1.0, 20.0, 35.0, 25, "Мангровые заросли", "Круглый Год", "Мелкая рыба,Креветка", 6, None),
                ("Мангровая рыба-камень", "Мифическая", 0.1, 0.5, 15.0, 25.0, 300, "Мангровые заросли", "Круглый Год", "Рыба,Креветка", 6, None),
                ("Мангровая скорпена", "Редкая", 0.05, 0.2, 10.0, 20.0, 25, "Мангровые заросли", "Круглый Год", "Рачки,Черви,Мотыль", 6, None),
                ("Мангровый бычок-кругляк", "Обычная", 0.005, 0.02, 3.0, 6.0, 10, "Мангровые заросли", "Круглый Год", "Черви,Рачки,Мотыль", 6, None),
                ("Мангровая зеленушка-радужная", "Редкая", 0.02, 0.06, 6.0, 10.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Рачки,Черви,Водоросли", 6, None),
                ("Мангровая рыба-жаба (бородавчатка)", "Редкая", 0.1, 0.3, 10.0, 18.0, 25, "Мангровые заросли", "Круглый Год", "Рыба,Креветка", 6, None),
                ("Мангровая рыба-флейта", "Редкая", 0.02, 0.08, 8.0, 15.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Рачки,Мелкая рыба", 6, None),
                ("Мангровая рыба-труба", "Редкая", 0.01, 0.04, 5.0, 12.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Рачки,Черви", 6, None),
                ("Мангровая рыба-игла", "Обычная", 0.01, 0.03, 6.0, 15.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Мелкие рачки,Планктон", 6, None),
                ("Мангровая рыба-полурыл", "Обычная", 0.01, 0.04, 5.0, 12.0, 10, "Мангровые заросли", "Весна,Лето,Осень", "Планктон,Насекомые", 6, None),
                ("Мангровая рыба-летучка", "Редкая", 0.005, 0.02, 3.0, 6.0, 25, "Мангровые заросли", "Весна,Лето,Осень", "Насекомые,Планктон", 6, None),
                ("Мангровая креветка-чистильщик", "Обычная", 0.005, 0.01, 2.0, 4.0, 10, "Мангровые заросли", "Круглый Год", "Паразиты,Детрит", 6, None),
                ("Мангровый краб-паук", "Обычная", 0.02, 0.08, 3.0, 8.0, 10, "Мангровые заросли", "Круглый Год", "Падаль,Водоросли", 6, None),
                ("Мангровый краб-вампир", "Редкая", 0.01, 0.04, 3.0, 6.0, 25, "Мангровые заросли", "Круглый Год", "Падаль,Черви,Моллюск", 6, None),
                ("Мангровая актиния ковровая", "Обычная", 0.01, 0.05, 3.0, 10.0, 10, "Мангровые заросли", "Круглый Год", "Планктон,Мотыль", 6, None),
                ("Мангровая морская звезда", "Обычная", 0.02, 0.08, 5.0, 12.0, 10, "Мангровые заросли", "Круглый Год", "Моллюск,Детрит", 6, None),
                ("Мангровый морской еж", "Обычная", 0.01, 0.03, 3.0, 6.0, 10, "Мангровые заросли", "Круглый Год", "Водоросли,Обрастания", 6, None),
            ...
            def parse_metric_range(range_value: str, default=(0.01, 0.1)):
                if not range_value or "неприменимо" in range_value.lower():
                    return default
                match = re.search(r"([\d.]+)\s*-\s*([\d.]+)", range_value)
                if not match:
                    return default
                return float(match.group(1)), float(match.group(2))

            def estimate_fish_base_price(rarity: str, max_weight: float) -> int:
                rarity_base = {
                    "Обычная": 12,
                    "Редкая": 55,
                    "Легендарная": 280,
                ...
                base = rarity_base.get(rarity, 12)
                if max_weight <= 0.05:
                    multiplier = 0.85
                elif max_weight <= 0.5:
                    multiplier = 1.0
                elif max_weight <= 5:
                    multiplier = 1.2 + max_weight * 0.12
                elif max_weight <= 30:
                    multiplier = 1.4 + max_weight * 0.06
                elif max_weight <= 100:
                    multiplier = 1.6 + max_weight * 0.025
                else:
                    multiplier = 2.0 + max_weight * 0.004
                return max(3, int(round(base * multiplier)))

            def estimate_max_rod_weight(max_weight: float) -> int:
                if max_weight <= 0.02:
                    return 2
                if max_weight <= 0.1:
                    return 4
            ...
            def build_fish_entry_from_info(name: str, info: dict) -> tuple:
                habitat = (info.get("habitat") or "").strip()
                location = next(
                    (prefix for prefix in NEW_LOCATION_PREFIXES if habitat.startswith(prefix)),
                    None,
                )
                if not location:
                    raise ValueError(f"Unknown habitat for {name}: {habitat}")
                rarity = RARITY_TITLE_MAP.get(
                    (info.get("rarity") or "обычная").strip().lower(),
                    "Обычная",
                )
                min_weight, max_weight = parse_metric_range(info.get("weight_range", ""))
                min_length, max_length = parse_metric_range(info.get("size_range", ""), (5, 15))
                seasons = normalize_seasons(info.get("seasons", ""))
                baits = normalize_baits(info.get("nutrition", ""))
                price = estimate_fish_base_price(rarity, max_weight)
                rod_weight = estimate_max_rod_weight(max_weight)
                return (
                    name, rarity, min_weight, max_weight, min_length, max_length, price,
                    location, seasons, baits, rod_weight, None,
                )

            existing_fish_names = {entry[0] for entry in fish_data}
            for fish_name, fish_info in FISH_INFO.items():
                if fish_name in existing_fish_names:
                    continue
                habitat = (fish_info.get("habitat") or "").strip()
                if not any(habitat.startswith(prefix) for prefix in NEW_LOCATION_PREFIXES):
                    continue
                fish_data.append(build_fish_entry_from_info(fish_name, fish_info))
            ...
            for entry in fish_data:
                ...
                normalized_fish_data.append((
                    ...
                    get_activity_for_fish_name(name, rarity),
                ))
            fish_data = normalized_fish_data
            
            cursor.execute('DELETE FROM fish')
            cursor.executemany('''
                INSERT OR REPLACE INTO fish (name, rarity, min_weight, max_weight, min_length, max_length, price, locations, seasons, suitable_baits, max_rod_weight, required_level, sticker_id, activity_period)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', fish_data)
    ...
    def get_all_players(self) -> List[Dict[str, Any]]:
        """Получить список всех игроков (глобальные профили)."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("PRAGMA table_info(players)")
            cols = [c[1] for c in cursor.fetchall()]
            ...
            if 'chat_id' in cols:
                # Выбираем только глобальные профили
                cursor.execute('''
                    SELECT * FROM players 
    ...
    def get_fish_by_location(
        ...
        with self._connect() as conn:
            ...
            if apply_time_filter:
                result = filter_fish_by_time(result)
            return result

    def get_fish_by_location_any_season(
        self,
        location: str,
        min_level: Optional[int] = None,
        apply_time_filter: bool = True,
    ...
    def add_caught_fish(self, user_id: int, chat_id: int, fish_name: str, weight: float, location: str, length: float = 0):
        ...
        with self._connect() as conn:
            ...
            try:
                self.evaluate_achievements(int(user_id), chat_id=chat_id_to_store)
            except Exception:
                logger.exception("evaluate_achievements failed user_id=%s", user_id)
    ...
    def add_bait(self, user_id: int, bait_name: str, quantity: int = 1) -> bool:
        """Добавить наживку игроку."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute('''
            ...
            ''', (user_id, bait_name, quantity, quantity))
            conn.commit()
            return True
    
    def remove_bait(self, user_id: int, bait_name: str, quantity: int = 1) -> bool:
        """Удалить наживку у игрока."""
        with self._connect() as conn:
            cursor = conn.cursor()
            ...
            cursor.execute('''
                ...
                WHERE user_id = ? AND bait_name = ?
            ''', (quantity, user_id, bait_name))
            
            # Удаляем если стало 0
            cursor.execute('''
                DELETE FROM player_baits 
                WHERE user_id = ? AND bait_name = ? AND quantity <= 0
            ''', (user_id, bait_name))
            
            conn.commit()
            return True
    
    def get_all_rods(self) -> List[Dict[str, Any]]:
        """Получить список всех удочек."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM rods ORDER BY price ASC')
            rows = cursor.fetchall()
            columns = [d[0] for d in cursor.description]
            return [dict(zip(columns, row)) for row in rows]
    ...
    def get_baits_for_location(self, location: str) -> List[Dict[str, Any]]:
        ...
        with self._connect() as conn:
            ...
            # Получаем наживки для этих рыб
            placeholders = ','.join('?' * len(fish_names))
            cursor.execute(f'''
                SELECT DISTINCT b.* FROM baits b
                JOIN fish_baits fb ON b.name = fb.bait_name
                WHERE fb.fish_name IN ({placeholders})
                ORDER BY b.name
            ''', fish_names)
    ...
    def add_clothing(self, user_id: int, clothing_code: str) -> bool:
        """Добавить одежду игроку."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT OR IGNORE INTO player_clothing (user_id, clothing_code)
                VALUES (?, ?)
            ''', (user_id, clothing_code))
            conn.commit()
            return True
    ...
    def add_project_donation(
        ...
        """Записать пожертвование проекту."""
        if not telegram_payment_charge_id or int(amount or 0) <= 0:
            return False
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                '''
                INSERT INTO project_donations (user_id, username, amount, telegram_payment_charge_id)
                VALUES (?, ?, ?, ?)
                ON CONFLICT (telegram_payment_charge_id) DO NOTHING
                ''',
                (int(user_id), username, int(amount), telegram_payment_charge_id),
            )
            conn.commit()
            return cursor.rowcount > 0

    def get_project_donations_total(self) -> int:
        """Сумма всех пожертвований проекту (Stars)."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT COALESCE(SUM(amount), 0) FROM project_donations')
            row = cursor.fetchone()
            return int(row[0] or 0) if row else 0

    def get_user_project_donations_total(self, user_id: int) -> int:
        """Сумма пожертвований конкретного пользователя."""
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'SELECT COALESCE(SUM(amount), 0) FROM project_donations WHERE user_id = ?',
                (int(user_id),),
            )
            row = cursor.fetchone()
            return int(row[0] or 0) if row else 0
    ...
    def grant_rod(self, user_id: int, rod_name: str, chat_id: int, stack: bool = False) -> bool:
        """
        Выдать пользователю удочку (глобально). 
        
        Args:
            user_id: ID пользователя
            rod_name: Название удочки
            chat_id: ID чата
            stack: Если True и удочка временная, прибавляет использования к существующей
        
        Returns:
            True если удочка выдана успешно
        """
        rod = self.get_rod(rod_name)
        if not rod:
            return False
        ...
        with self._connect() as conn:
            ...
            # Определяем использования для временной удочки
            new_uses = self._get_temp_rod_uses(rod_name)
            is_temp_rod = new_uses is not None
            
            if existing:
                current_dur, max_dur = existing
                
                # Если это временная удочка и включен стакинг
                if is_temp_rod and stack:
                    # Прибавляем новые использования к текущим
                    new_current = int(current_dur) + new_uses
                    new_max = int(max_dur) + new_uses
                    
                    cursor.execute('''
                        UPDATE player_rods
                        SET current_durability = ?, max_durability = ?
                        WHERE user_id = ? AND (chat_id IS NULL OR chat_id < 1) AND rod_name = ?
                    ''', (new_current, new_max, user_id, rod_name))
                else:
                    # Обычная логика - восстанавливаем до полной прочности
                    if is_temp_rod:
                        max_dur = new_uses
                        current = new_uses
                    else:
